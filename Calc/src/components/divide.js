...
import {divide} from '../actions/index';
...
class Divide extends Component{
    ...
    if(operatorStack.length != 0){
      ...
      if(lastElem.length == 0){
        if(operatorStack[operatorStack.length-2] && isNaN(operatorStack[operatorStack.length-2])){
          ...
          this.props.divide(expString);
        }
      }else{
        this.props.divide(this.props.calculate.expression + " ÷ ")
      }
    }    
  }
  ...
  render(){
    return(
    <View>
      {/* <Text onPress={()=>this.props.divide(this.props.calculate.expression + " / ")}>{"Divide"}</Text> */}
...
function matchDispatchToProps(dispatch){
    return bindActionCreators({divide: divide}, dispatch)
}

export default connect(mapStateToProps, matchDispatchToProps)(Divide);