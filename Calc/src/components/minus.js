...
class Minus extends Component{
    ...
    if(operatorStack.length != 0){
      ...
      if(lastElem.length == 0){
        if(operatorStack[operatorStack.length-2] && isNaN(operatorStack[operatorStack.length-2])){
          ...
          this.props.minus(expString);
        }
      }else{
        this.props.minus(this.props.calculate.expression + " - ")
      }
    }    
  }
  ...
  render(){
    return(
    <View>
      {/* <Text onPress={()=>this.props.minus(this.props.calculate.expression + " - ")}>{"Minus"}</Text> */}
...
export default connect(mapStateToProps, matchDispatchToProps)(Minus);